...
pub struct TrayState {
    ...
    /// Câblé depuis main.rs plutôt que d'importer ui::popup_menu ici
    /// directement -- ce module garde son propre WndProc (clic gauche =
    /// `super::window::toggle`, seul appel direct à un autre module ui::)
    /// mais n'a pas besoin de connaître le contenu/style du menu lui-même.
...
/// Même principe que `ui::window::get_state` -- récupère `TrayState` depuis
/// `GWLP_USERDATA`. Exposée pour que main.rs puisse réenregistrer le hotkey
/// depuis un hook posé sur la fenêtre principale (voir
/// `window::set_hotkey_reload_hook`), le seul appelant hors de ce fichier.
pub(crate) unsafe fn get_state<'a>(hwnd: HWND) -> Option<&'a mut TrayState> {
...
/// "hotkey_enabled" persisté dans state.json (voir core::state) : le hotkey
