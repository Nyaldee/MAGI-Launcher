...
pub unsafe fn show(parent: HWND, colors: MenuColors, font_family: &str, border_width: i32, items: Vec<MenuItem>) -> Option<String> {
    ...
    // Coin bas-droit du menu au curseur (le menu s'ouvre vers le HAUT et la
    // GAUCHE) -- plus naturel pour un menu de tray, ancré en bas-droite de
    // l'écran : le clic reste au même endroit sans avoir à traverser tout
    // l'écran pour atteindre le premier item.
    let x = (cursor.x - window_w).max(0).min(screen_w - window_w);
    let y_pos = (cursor.y - window_h).max(0).min(screen_h - window_h);
