...
//! La lecture couvre tout ce dont apps.json/themes.json/state.json ont
//! besoin (objets/tableaux imbriqués, chaînes avec échappements, nombres,
//! booléens, null). L'écriture couvre le tableau plat de chaînes utilisé par
//! notes.json/restart.json (voir core::json_list) et l'objet imbriqué de
//! state.json (voir core::state) -- les deux seuls fichiers que le lanceur
//! réécrit lui-même, jamais édités à la main. apps.json/themes.json restent
//! en lecture seule côté app : purement édités à la main par l'utilisateur.
//!
...
impl Json {
    pub fn parse(text: &str) -> Result<Json, ParseError> {
        // Certains éditeurs (bloc-notes hérité, PowerShell -Encoding utf8)
        // écrivent un BOM UTF-8 en tête -- apps.json/themes.json étant édités
        // à la main, l'ignorer plutôt que de rejeter tout le fichier.
...
/// est laissé en UTF-8 littéral. Publique car core::state écrit des valeurs
/// de chaîne arbitraires (nom de thème, hotkey...) en réécrivant state.json
/// en entier à chaque sauvegarde.
...
mod tests {
    ...
    fn parse_ignore_un_bom_utf8_en_tete() {
        let v = Json::parse("\u{FEFF}{\"a\": 1}").unwrap();
