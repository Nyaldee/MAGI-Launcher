...
//! (load_list/save_list) et ses emballages de domaine, notes.json (Sticky
//! Notes), restart.json (liste Auto-restart) et kill.json (liste Auto-kill)
//! -- les seuls fichiers de config avec cette forme (juste le tableau
//! lui-même, pas d'objet englobant). Regroupés dans un seul fichier plutôt
//! qu'un module par emballage : aucun n'a de logique propre à ajouter,
//! chacun ne fait que renommer load_list/save_list.
//!
//! Le lanceur est normalement seul à les écrire, mais Reload (voir
//! ui::window::reload_config) les relit depuis le disque pour couvrir une
//! modification manuelle faite pendant qu'il tourne.
...
pub fn save_list(path: &Path, items: &[String]) -> std::io::Result<()> {
    // Une `String` Rust est toujours de l'UTF-8 valide : aucun nettoyage
    // de substituts UTF-16 isolés n'est nécessaire avant écriture.
...
/// `path` au sens d'une entrée apps.json (arguments compris), comme
/// restart.json -- voir core::supervisor pour la résolution de la cible.
pub fn load_kill_list(path: &Path) -> Vec<String> {
    load_list(path)
}

pub fn save_kill_list(path: &Path, targets: &[String]) -> std::io::Result<()> {
    save_list(path, targets)
}
