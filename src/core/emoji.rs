...
//!
//! Aucune table d'emoji embarquée dans le binaire : ce fichier texte est
//! remplaçable par une version plus récente d'Unicode sans recompiler
//! (voir Reload). Format d'une ligne de données :
//! `<codepoints hex> ; <statut> # <glyphe> <tag de version> <nom>` -- le
//! glyphe rendu figure déjà tel quel dans le commentaire, inutile de
//! recomposer les points de code.
...
/// `None` sur une ligne qui n'est pas "fully-qualified" -- les autres
/// statuts (component/minimally-qualified/unqualified) sont des briques
/// internes ou des doublons du même emoji -- ou dont le format est
/// inattendu : une telle ligne est ignorée, sans invalider le reste du
/// fichier.
...
";
    ...
    fn charge_le_vrai_emoji_test_txt_du_projet() {
        // Contre le fichier réellement livré à côté de l'exe, pas
        // l'échantillon synthétique de SAMPLE.
        ...
        // Seuil large plutôt qu'un compte exact : robuste à une mise à
        // jour du fichier vers une version Unicode ultérieure.
