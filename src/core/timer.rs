...
/// Garde-fou sur la durée analysée -- protège le `SetTimer` du minuteur
/// d'un débordement, avec une limite volontairement large (~3000 ans).
...
pub fn parse_duration(text: &str) -> Option<u64> {
    ...
    let rest = rest.trim_start(); // espaces tolérés entre les chiffres et l'unité
