...
///
/// La recherche de plus petite fenêtre n'est PAS juste "ancrer chaque
/// lettre sur sa première occurrence puis mesurer" -- ancrer trop tôt peut
/// rater une occurrence bien plus tardive et bien plus resserrée de la
/// même lettre plus loin dans le nom. Exemple : query="cp" sur "Acronis
/// Cyber Protect" -- ancrer 'c' sur sa première occurrence ("acronis")
/// donne une plage de 13 jusqu'au seul 'p' ("protect"), alors que le 'c' de
/// "Cyber" (bien plus proche du 'p') donne une plage de 6, la vraie plus
/// petite. Solution : pour CHAQUE occurrence du DERNIER caractère de la
/// requête, chercher en arrière (rfind, donc la plus proche/tardive
/// occurrence de chaque caractère précédent) la plus petite fenêtre qui se
/// termine précisément là, puis garder le minimum sur toutes ces fins
/// possibles -- chercher en arrière depuis une fin déjà fixée maximise
/// chaque position intermédiaire, ce qui minimise la plage pour CETTE fin
/// (preuve par échange standard), et balayer toutes les fins possibles
/// couvre le cas (illustré plus haut) où une fin plus tardive permet une
/// plage globalement plus petite qu'une fin plus précoce.
pub fn match_rank(name_lower: &str, query_lower: &str) -> Option<(u8, usize)> {
    ...
    loop {
        let end = match name[search_from..].iter().position(|&c| c == last_char) {
        ...
        if matched {
            let span = end - pos;
            if best_span.map_or(true, |b| span < b) {
                best_span = Some(span);
            }
        }
        search_from = end + 1;
...
/// UN SEUL mot ne correspond nulle part. C'est le point d'entrée réellement
/// utilisé par ui::window (voir fuzzy_filter) ; match_rank() reste le bloc
/// de base réutilisé par mot -- une requête sans espace se comporte
/// exactement comme avant (split_whitespace() sur un seul mot renvoie ce
/// mot tel quel, la boucle ne tourne qu'une fois).
///
...
/// Minuscule + repli des accents latins courants (français) vers leur forme
/// ASCII de base -- pas une vraie décomposition NFD/table Unicode complète
/// (hors de proportion ici), une liste couvrant ce qu'on rencontre
/// réellement en usage francophone (noms d'applis/fenêtres...). À appliquer
/// aux DEUX côtés d'une comparaison (voir core::models::App::new et
/// ui::window) : sans repli symétrique, une requête "credit" ne
/// retrouverait jamais un nom "Crédit Agricole" (é != e caractère pour
/// caractère).
...
mod tests {
    ...
    fn plus_petite_fenetre_prefere_l_occurrence_tardive_plus_serree() {
        // Exemple travaillé ci-dessus : un ancrage naïf sur la
        // première occurrence donnerait une plage de 13
    ...
    fn match_rank_multi_exige_chaque_mot_independamment_de_l_ordre() {
        // "code visual" ne matcherait JAMAIS via match_rank() seul (l'ordre
        // "code" avant "visual" casse la sous-séquence stricte de gauche à
        // droite) -- ici chaque mot est cherché indépendamment, donc ça
        // matche malgré l'ordre inversé.
