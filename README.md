...
- Auto-kill — the mirror of Auto-restart: pick targets to keep dead, terminated on sight every couple seconds. Off by default, with a fixed 10-second grace after each enable and a hard "never kill MAGI itself" rule
...
- Runs as a single instance with a tray icon (toggle hotkey / toggle auto-restart / toggle auto-kill / toggle Copy History / GitHub / quit)
...
| `Shift+Delete` | *(Window Switcher)* Force-kill the highlighted window's process, for one that stopped responding to `Delete` — *(Sticky Notes)* Delete every note — *(Auto-restart)* Stop watching every target — *(Auto-kill)* Clear every kill target — *(Copy History)* Clear the whole history — *(Recycle Bin browse view)* Empty the whole bin (launcher stays open) — *(Eject)* Force-eject the highlighted drive even if something still has it open; plain `Delete` does nothing here |
| `Ctrl+1`–`Ctrl+9` / `Ctrl+0` | Resize the popup live to 10%–90% / 100% of screen width, persisted to `state.json` immediately |
| `Ctrl+-` / `Ctrl+=` | Shrink / grow the border by 1px, live, persisted immediately |
| `Backspace` on an empty search | Back out of the Window Switcher / Theme Picker / Timer / Sticky Notes / Auto-restart / Auto-kill / Recycle Bin browse view / Emoji picker / Copy History / Eject |
| Left-click tray icon | Show / hide the launcher |
| Right-click tray icon | Toggle hotkey / Toggle auto-restart / Toggle auto-kill / Toggle Copy History / GitHub / Quit menu |
...
The mirror image of Auto-restart, and it shares the same background thread: a `"magi:auto-kill"` entry unlocks a list of targets (stored in `kill.json`) that MAGI keeps **dead** — every couple seconds it terminates any of them it finds running (all instances of that executable name). The picker works identically to Auto-restart's — `Auto-kill: N` in the main list, fuzzy-searchable, each target prefixed with `✖` (running, will be terminated) or `·` (not running) — and the placeholder is `Type a target to kill...`. `Enter` adds a new target, `Delete` removes the highlighted one, `Shift+Delete` clears the list.

Two safety rules, both non-negotiable and not configurable:

- **10-second grace.** Nothing is killed for the first 10 seconds after Auto-kill is enabled — every time: at MAGI startup, and every time you toggle it on from the tray. If `kill.json` points at something you actually need, that's your window to open the tray and pick "Disable Auto-kill" before the first kill ever fires.
- **MAGI never kills itself.** `magi_launcher.exe` is excluded no matter what `kill.json` says — otherwise a bad list plus a launch-at-startup would kill MAGI on every boot, leaving no tray to disable it from.

Auto-kill defaults to **off** (unlike Auto-restart, which is on by default): a feature that terminates processes has to be turned on deliberately. Its tray toggle and `auto_kill_enabled` in `state.json` both persist the choice; re-enabling either way re-arms the 10-second grace.
...
The `hotkey_enabled`/`auto_restart_enabled`/`auto_kill_enabled`/`copy_history_enabled` state of the tray toggles is persisted to `state.json` (see below) the moment you flip them, so they survive a restart.
...
`state.json` lives there too, but it's a different kind of file — internal bookkeeping (active theme, font/window/border preferences, and the three tray toggles) that the launcher itself reads once at startup and rewrites automatically the moment you change one of those from the keyboard/tray/theme picker. Not meant to be hand-edited during a run for the same reason as `notes.json`/`restart.json` below, but see [`state.json`](#statejson) if you want to set an initial value before first launch, or just look at what's currently persisted.

`notes.json`/`restart.json` live there too, but they're a different kind of file: `notes.json` is a plain JSON array of strings (`["note 1", "note 2"]`), `restart.json` a plain JSON array of `path` strings (`["A:\\Apps\\Foo\\Foo.exe"]`) — both created and rewritten automatically by the app itself whenever you add or remove an entry in Sticky Notes/Auto-restart, so the launcher's own in-memory copy is normally always the source of truth. Not meant to be hand-edited, but **Reload** re-reads both from disk too (alongside `apps.json`/`themes.json`/`state.json`), so a manual edit to either still gets picked up without restarting.
...
| `path` | Effect |
|---|---|
| `magi:reload` | Reload `apps.json`/`themes.json`/`state.json`/`notes.json`/`restart.json`/`kill.json`/`emoji-test.txt`/`shortcuts/` in place, no restart |
...
| `magi:auto-kill` | Enter the Auto-kill picker (see above); shows `<name>: N` for the number of kill targets, `0` when empty |
...
Just the color palettes themselves — which one is active, and every other display preference, lives in [`state.json`](#statejson) below instead (kept separate so replacing `themes.json` wholesale with a newer palette set never resets your own preferences).

Ships with 100+ built-in themes (mostly character/game palettes) in the `themes` dict — open the launcher and select the `Themes` entry (`magi:theme-picker`) to preview and switch between them live, no restart required. The picker opens on whichever theme is currently active, not the first one alphabetically.

### `state.json`

Internal bookkeeping that MAGI Launcher writes to on its own — you shouldn't need to touch it, but it's plain JSON if you want to look, or set an initial value before first launch:
...
- **`ui.theme`** — name of the active entry in `themes.json`'s `themes` dict. Written the moment you pick one from the theme picker.
- **`ui.font_family`** — `null` to keep the OS default font. Recommended font: [SGr-Iosevka-Regular.ttc](https://github.com/be5invis/iosevka)
- **`ui.placeholder_text`** — shown in the search box while empty (overridden by a mode-specific placeholder in Timer/Sticky Notes/Auto-restart/Auto-kill, see their sections above)
- **`ui.show_clock`** — show the current time (in the user's Windows short time format) next to the search box
- **`ui.window_size`** — percentage (0–100) of the screen width the popup occupies (height/font sizes follow, keeping a 16:9 ratio, always centered on the monitor under the cursor and clamped to never run off-screen). Adjustable live from the keyboard too, see `Ctrl+1`–`Ctrl+0` above — each press writes the new value straight back here.
- **`ui.border`** — simulated border thickness in pixels. Adjustable live too, see `Ctrl+-`/`Ctrl+=` above.
...
- **`hotkey_enabled`** / **`auto_restart_enabled`** / **`auto_kill_enabled`** / **`copy_history_enabled`** — mirror the tray menu toggles, read once at startup and rewritten automatically the moment you flip one from the tray. `auto_kill_enabled` defaults to `false` (the others to `true`) — a process-killer is opt-in; enabling it (here or from the tray) starts a fixed 10-second grace before the first kill.
