...
- Auto-kill — le miroir d'Auto-restart : choisis des cibles à garder mortes, terminées à vue toutes les quelques secondes. Désactivé par défaut, avec une grâce fixe de 10 secondes après chaque activation et une règle stricte « ne jamais tuer MAGI lui-même »
...
- Tourne en instance unique avec une icône dans la zone de notification (activer/désactiver le hotkey / activer/désactiver l'auto-restart / activer/désactiver l'auto-kill / activer/désactiver Copy History / GitHub / quitter)
...
| `Shift+Suppr` | *(Window Switcher)* Tue de force le process de la fenêtre en surbrillance, pour une qui ne répond plus à `Suppr` — *(Sticky Notes)* Supprime toutes les notes — *(Auto-restart)* Arrête de surveiller toutes les cibles — *(Auto-kill)* Vide toutes les cibles à tuer — *(Copy History)* Vide tout l'historique — *(Corbeille, vue de consultation)* Vide toute la Corbeille (le lanceur reste ouvert) — *(Eject)* Force l'éjection du disque en surbrillance même si quelque chose l'utilise encore ; `Suppr` seul ne fait rien ici |
| `Ctrl+1`–`Ctrl+9` / `Ctrl+0` | Redimensionne le popup en direct à 10%–90% / 100% de la largeur de l'écran, persisté aussitôt dans `state.json` |
| `Ctrl+-` / `Ctrl+=` | Réduit / agrandit la bordure de 1px, en direct, persisté aussitôt |
| `Retour arrière` sur une recherche vide | Sort du Window Switcher / du sélecteur de thème / du Timer / des Sticky Notes / d'Auto-restart / d'Auto-kill / de la vue de consultation de la Corbeille / du sélecteur d'emoji / de Copy History / d'Eject |
| Clic gauche sur l'icône tray | Afficher / masquer le lanceur |
| Clic droit sur l'icône tray | Menu Activer/désactiver le hotkey / Activer/désactiver l'auto-restart / Activer/désactiver l'auto-kill / Activer/désactiver Copy History / GitHub / Quitter |
...
L'image miroir d'Auto-restart, et il partage le même thread de fond : une entrée `"magi:auto-kill"` débloque une liste de cibles (stockée dans `kill.json`) que MAGI garde **mortes** — toutes les quelques secondes il termine celles qu'il trouve en cours (toutes les instances de ce nom d'exécutable). Le picker fonctionne à l'identique de celui d'Auto-restart — `Auto-kill: N` dans la liste principale, cherchable de façon floue, chaque cible préfixée de `✖` (en cours, sera terminée) ou `·` (pas en cours) — et le placeholder est `Type a target to kill...`. `Entrée` ajoute une cible, `Suppr` retire celle en surbrillance, `Shift+Suppr` vide la liste.

Deux règles de sécurité, toutes deux non négociables et non configurables :

- **Grâce de 10 secondes.** Rien n'est tué pendant les 10 premières secondes après l'activation d'Auto-kill — à chaque fois : au démarrage de MAGI, et à chaque fois que tu l'actives depuis le tray. Si `kill.json` vise quelque chose dont tu as réellement besoin, c'est ta fenêtre pour ouvrir le tray et choisir « Disable Auto-kill » avant le premier kill.
- **MAGI ne se tue jamais lui-même.** `magi_launcher.exe` est exclu quoi que dise `kill.json` — sinon une liste fautive plus un lancement au démarrage tuerait MAGI à chaque boot, sans tray pour l'arrêter.

Auto-kill est **désactivé** par défaut (contrairement à Auto-restart, actif par défaut) : une fonction qui termine des processus doit être activée délibérément. Sa bascule tray et `auto_kill_enabled` dans `state.json` persistent le choix ; la réactiver par l'un ou l'autre réarme la grâce de 10 secondes.
...
L'état des bascules du tray (`hotkey_enabled`/`auto_restart_enabled`/`auto_kill_enabled`/`copy_history_enabled`) est persisté dans `state.json` (voir plus bas) dès que tu les changes, pour qu'elles survivent à un redémarrage.
...
`state.json` vit là aussi, mais c'est un genre de fichier différent — de la comptabilité interne (thème actif, préférences de police/fenêtre/bordure, et les trois bascules du tray) que le lanceur lit lui-même une fois au démarrage et réécrit automatiquement dès que tu changes l'un de ces réglages depuis le clavier/le tray/le sélecteur de thème. Pas fait pour être édité à la main pendant que le lanceur tourne, pour la même raison que `notes.json`/`restart.json` plus bas, mais voir [`state.json`](#statejson) si tu veux fixer une valeur de départ avant le premier lancement, ou juste regarder ce qui y est persisté.

`notes.json`/`restart.json` vivent là aussi, mais c'est un genre de fichier différent : `notes.json` est un simple tableau JSON de chaînes (`["note 1", "note 2"]`), `restart.json` un simple tableau JSON de chaînes `path` (`["A:\\Apps\\Foo\\Foo.exe"]`) — les deux créés et réécrits automatiquement par l'appli elle-même à chaque ajout/suppression d'entrée dans Sticky Notes/Auto-restart, donc la copie en mémoire du lanceur fait normalement toujours foi. Pas faits pour être édités à la main, mais **Reload** relit aussi les deux depuis le disque (en plus de `apps.json`/`themes.json`/`state.json`), donc une modification manuelle de l'un ou l'autre est quand même prise en compte sans redémarrer.
...
| `path` | Effet |
|---|---|
| `magi:reload` | Recharge `apps.json`/`themes.json`/`state.json`/`notes.json`/`restart.json`/`kill.json`/`emoji-test.txt`/`shortcuts/` sur place, sans redémarrage |
...
| `magi:auto-kill` | Entre dans le picker Auto-kill (voir plus haut) ; affiche `<nom>: N` pour le nombre de cibles à tuer, `0` si vide |
...
Juste les palettes de couleurs elles-mêmes — quel thème est actif, et toute autre préférence d'affichage, vit dans [`state.json`](#statejson) plus bas à la place (gardé séparé pour qu'un remplacement complet de `themes.json` par un nouveau jeu de palettes ne réinitialise jamais tes propres préférences).
...
Comptabilité interne que MAGI Launcher écrit lui-même — tu ne devrais pas avoir besoin d'y toucher, mais c'est du JSON tout ce qu'il y a de plus simple si tu veux regarder, ou fixer une valeur de départ avant le premier lancement :
...
- **`ui.theme`** — nom de l'entrée active dans le dict `themes` de `themes.json`. Écrit dès que tu en choisis un dans le sélecteur de thème.
- **`ui.font_family`** — `null` pour garder la police par défaut de l'OS. Police recommandée : [SGr-Iosevka-Regular.ttc](https://github.com/be5invis/iosevka)
- **`ui.placeholder_text`** — affiché dans la barre de recherche quand elle est vide (remplacé par un placeholder spécifique au mode dans Timer/Sticky Notes/Auto-restart/Auto-kill, voir leurs sections plus haut)
- **`ui.show_clock`** — affiche l'heure actuelle (au format court de Windows de l'utilisateur) à côté de la barre de recherche
- **`ui.window_size`** — pourcentage (0–100) de la largeur de l'écran occupée par le popup (hauteur/tailles de police suivent, en gardant un ratio 16:9, toujours centré sur le moniteur sous le curseur et jamais laissé déborder de l'écran). Ajustable en direct au clavier aussi, voir `Ctrl+1`–`Ctrl+0` plus haut — chaque pression réécrit aussitôt la valeur ici.
- **`ui.border`** — épaisseur de bordure simulée en pixels. Ajustable en direct aussi, voir `Ctrl+-`/`Ctrl+=` plus haut.
...
- **`hotkey_enabled`** / **`auto_restart_enabled`** / **`auto_kill_enabled`** / **`copy_history_enabled`** — reflètent les bascules du menu tray, lues une fois au démarrage et réécrites automatiquement dès que tu en bascules une depuis le tray. `auto_kill_enabled` vaut `false` par défaut (les autres `true`) — un tueur de processus est opt-in ; l'activer (ici ou depuis le tray) démarre une grâce fixe de 10 secondes avant le premier kill.
